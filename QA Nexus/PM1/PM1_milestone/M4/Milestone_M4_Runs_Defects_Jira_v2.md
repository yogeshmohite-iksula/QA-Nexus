...
- **2026-05-14 15:02 IST · #146 merged → main `8e79e87`** — M4 v2 plan officially live. Conflict resolution: kept Day-18 PM ADR-019 entries (mine) + Day-18 AM #147 `/health` 2-tier refactor + Day-18 AM #144 migration 0004 entries (Yogesh's) in CHANGELOG; merged (bw)/(br)/(bq) followups chronologically. PR #146 final SHA `8e79e87` includes 5 commits squashed.
- **2026-05-14 15:36 IST · #150 visual gate GREEN (FE+1 F20 fix-forward)** — F20 Run Results Pattern A landed via verbatim Hard Rule 17 re-port. All 22 design tokens reused (no invented hex), all user-visible strings traced to `F20 Run Results v2.html` via `canned-data.ts` extracted by `scripts/extract-canned-data.mjs`. Side-by-side visual gate passed at 1440. **Rule 17 working as designed** — caught the invention class on first attempt (#145), forced fix-forward to a clean port (#150).
- **2026-05-14 15:40 IST · #145 CLOSED (NOT MERGED) as Hard Rule 17 violation precedent** — `feat(web): f20 run results pattern a scaffold` closed with supersession comment pointing to #150. First case-study citation for Rule 17. Closure rationale: FE+1's initial attempt invented stub data (cluster titles, ticket IDs, right-rail labels) instead of extracting from the v2 HTML; visual gate caught the drift; fix-forward path was a clean re-port rather than patching the drift. This sets the precedent that Rule 17 violations are NOT merge-and-fix-later — they are close-and-redo. Filed followup `(bx)` P3 for the F20 v2 HTML 320px horizontal-scroll bug surfaced during the visual gate (cosmetic only — React port already reflows correctly per Hard Rule 12 RWD; canonical HTML needs a Claude Design pass to mirror the F19 Round-4 reflow pattern).
- **2026-05-14 ~15:30 IST · #148 WebSocket Gateway HOLDING for close cascade** — `feat(api): WebSocket gateway for real-time run events (M4 TASK 2)`. BE+1 implementation per MS4-T009 + §4.7 WebSocket event taxonomy (run.state_changed / run.result_recorded / run.completed / defect.created / defect.status_changed / defect.jira_linked / defect.sherlock_ready / defect.needs_review / jira.webhook_received). `@nestjs/websockets` + `ws` single-instance per stack lock §7. Awaiting Yogesh merge directive.
- **2026-05-14 ~15:30 IST · #149 TestRun service HOLDING** — `feat(api): TestRun service skeleton + state machine (M4 TASK 3 P3)`. BE+1 implementation per MS4-T004/T005 — Pattern A scaffold (501) + lifecycle state machine `pending → running → passed/failed/blocked`. Audit row on every transition per PM1_ERD §3.13. Awaiting Yogesh merge directive (TestRun service unblocks F19 Pattern B real wiring).
- **2026-05-14 ~16:00 IST · F21 Defects Hub Pattern A in progress (FE+1)** — Hard Rule 17 applied from the start (no #145-style invention pass). FE+1 ran `scripts/extract-canned-data.mjs --frame F21 --html "QA Nexus/PM1/PM1_UI_v2/Redesign Frame by claude design/F21 Defects Hub v2.html"` as Step 1. `apps/web/components/f21-defects-hub/canned-data.ts` generated. Component port consumes only extracted strings. ETA ~18:45 IST for PR open. Rule 17 normalizing across the team — first measurable trickle-down effect from the close-and-redo precedent set with #145.
- **2026-05-14 status · F19 Run Console Pattern B blocked on #148 close cascade** — Pattern A live from M3 close week; Pattern B needs WebSocket subscription to `/api/projects/:projectId/runs/:runId/stream`. Until #148 merges + Render redeploys + boot log confirms WebSocket gateway registered, FE+1's F19 Pattern B work can't progress past the canned-event-stream Pattern A path. This is the longest blocker chain on Day-18; everything else fans out behind it.
