...
**2026-04-28 implementation note (amended 2026-06-02 per ADR-003 Day-4 amendment + followup `(ae)`):** PM1 runs `Xenova/bge-small-en-v1.5` as the embedding model (384-dim, ONNX, Apache-2.0). The original `Xenova/bge-large-en-v1.5` (1024-dim) was swapped to bge-small on Day-4 to fit Render Free's 512 MB memory ceiling (bge-large OOM'd at ~470 MB resident — ADR-003 amendment + Day-5 `vector(384)` migration `0002_vector_384_dim.sql`). `Qwen3-Embedding-0.6B` (1024-dim) remains the eventual target once Xenova ships its ONNX conversion. Schema dimension is `vector(384)`; runtime API + HNSW indexes are unchanged across model swaps — hot-swap is a single env var (`EMBEDDING_MODEL_ID`). Other mentions of "bge-large"/"1024-dim"/"Qwen3-Embedding-0.6B" in this doc refer to the historical plan or future-target model. Full rationale + alternatives + future-swap path in `docs/architecture/adr-003-embedding-model.md`.
