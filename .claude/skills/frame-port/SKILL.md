...
- Section-by-section locator-count compare (canonical vs port). FAIL if any section is MISSING (in canonical, absent in port) or EXTRA (in port but absent in canonical).
- Pixel diff under sharp-based raw RGBA comparison. FAIL if >5% pixel diff at any viewport.

**Exit 0 = clean; Exit 1 = drift.**

If drift detected: orchestrator does NOT proceed to Step 6. Instead it returns the diff table to FE+1, who fixes the root cause and re-probes. NEVER patch the diff symptom (e.g. don't add a missing section just to pass the count check; understand WHY it was missing in the React tree first).
