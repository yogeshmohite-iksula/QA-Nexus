...
- @.claude/memory/feedback_institutional_memory_survives_in_vcs.md — BINDING: institutional memory must live in version control + off-device backup, never only in agent process state. Before any laptop/account/email rotation: anything that matters must be in (a) the cloned repo OR (b) an off-device copy of local-only stores (user auto-memory, Cowork workspace, chat-history). "The next session will remember" = lost. Thu Jun 18 prep for Sun Jun 21 transition → master handoff `docs/handoff/2026-06-21-laptop-transition-master-handoff.md` is the surviving brain. (49th RC candidate — ordinal collision with constraint-scope file below; both real, reconciliation pending Phase D)
- @.claude/memory/feedback_verify_constraint_scope_before_expensive_workaround.md — BINDING: when an external constraint (quota / rate limit / feature gate / tier cutoff) blocks your plan, verify its EXACT scope (per-account vs per-project vs per-region vs per-call) from primary vendor docs BEFORE proposing an expensive workaround. The cheap same-vendor alternative often lives in the unread next paragraph. Thu Jun 18: Yogesh's "why Supabase?" → re-reading Neon docs → 100 CU-hr cap is per-project not per-account → Path C = 2nd Neon project qa-nexus-2 ($0, ~1 hr) beats Supabase migration (3-4 hr, new vendor). #285 Supabase plan reframed as fallback. (49th RC candidate — see ordinal-collision note in file)
