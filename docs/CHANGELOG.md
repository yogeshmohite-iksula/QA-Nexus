...
### Added — Day 32 (Fri 2026-06-19) — GET /api/test-runs list endpoint (F08 /home ACTIVE_RUNS + RECENT_RUNS) [m5-hardening]

Unblocks the F08 /home ACTIVE_RUNS + RECENT_RUNS surfaces (previously `<ComingSoon>` stubs — no backing list endpoint existed; `TestRunsController` had only `@Patch` state-transition handlers, caught by FE+1's pre-wire contract read). New **`GET /api/test-runs`** `list` method on `TestRunsService` + `@Get()` on `TestRunsController`: workspace-scoped via `project.workspaceId` (the proven-live app-level isolation layer per the Day-32 G5 probe — cross-tenant rows invisible, no leak / no 403), `@UseGuards(RolesGuard)` + `@Roles(Admin, Lead, QAEngineer, Stakeholder)` (read parity with the defects / test-cases lists). Optional filters `?status=&projectId=&sort=&page=&pageSize=` — `status=running` drives ACTIVE_RUNS; default `sort=started_at_desc` (nulls-last, since queued runs have no `started_at`) drives RECENT_RUNS. **Offset** pagination (page / pageSize, max 50) — NOT the `/api/audit` cursor. Case counts (`totalCases` / `passedCases` / `failedCases`) are tallied from the joined `test_run_results` (the run row has no denormalized counts). **Reads are NOT audited** (ERD §8.7 + Hard Rule 7). `packages/shared/src/schemas/test-run.ts` gains `TestRunListItem` / `TestRunListQuery` / `TestRunListResponse` + `TestRunProjectRef` / `TestRunUserRef` (Hard Rule 10) — `triggeredBy` is the **nullable** human triggerer (webhook / cron runs → null), distinct from the `trigger` `RunTrigger` enum. No DB / migration impact (reads existing rows; the `test_runs` table is currently empty, so the list returns `{ ok: true, testRuns: [], pagination: { total: 0 } }` until the runner creates runs); requires a Render API redeploy. No DI change — `list` uses only the already-injected `PrismaService`. Tests (TDD): 7 service-spec cases (row → wire mapping + count tally, workspace scoping, status filter, projectId filter, sort + offset math, empty list, webhook/cron → `triggeredBy` null). Mirrors the W2-R defects read pattern.

### Fixed — Day 32 (Fri 2026-06-12) — gate report-aggregate crons to the pilot window (Neon CU-hr cap) [m5-hardening]

**🔴 incident:** the Neon free-tier compute cap (100 CU-hr) was hit and the project auto-suspended, blocking the pilot launch. **Root cause (code-verified):** `reports-refresh.cron.ts` ran an **unconditional `reportAggregate.updateMany` every 15 min, round-the-clock** (`@Cron('*/15 * * * *')`) plus an `02:30 IST` daily refresh — these reset Neon's 5-min autosuspend timer 24/7, so compute never scaled to zero. The original hypothesis (UptimeRobot's `/health` keep-alive waking Neon) was **refuted by code-reading**: `/health` is memory-only since Day-18 #146 (no Prisma call in the handler, and no global guard / interceptor / `$use` middleware touches the DB), so health pings never query Postgres. **Fix:** both crons are gated to the pilot window (Asia/Kolkata) — the stale sweep → `@Cron('*/15 10-21 * * *', { timeZone: 'Asia/Kolkata' })` (10:00–21:59 IST) and the daily refresh → `@Cron('0 10 * * *', { timeZone: 'Asia/Kolkata' })` (10:00 IST window-start, moved from 02:30). Neon now sleeps **22:00–10:00 IST (12h/day guaranteed)**. **Trade-off:** report aggregates may be stale during 22:00–10:00 IST — acceptable for the 8-user pilot (nobody queries reports overnight; the 10:00 refresh heals at window-open). Backs Hard Rule 1 ($0 cost gate). Also adds **`GET /health/lite`** (`HealthLiteController`, zero-dependency constructor → DB-free **by construction**) as one clearly-named keep-alive target for operators — belt-and-suspenders, **NOT** the fix (`/health` was already DB-free). Tests: a cron-schedule guard asserting the gated expressions (blocks a 24/7 regression) + `/health/lite` zero-injection + memory-only shape; 44 health+reports tests green; typecheck clean. No DB/migration impact; requires a Render API redeploy. **Operator follow-up:** confirm UptimeRobot targets `/health` (or `/health/lite`), never `/health/deep` (the only DB-touching health path).

### Added — Day 32 (Fri 2026-06-12) — W2-R defects read API (F21 Defects Hub) [m5-hardening]

Closes the Phase-B W2 🔴 finding (`docs/audits/2026-06-13-sat-be-workflow-conformance.md`): the 25 seeded defects were unreachable — `GET /api/defects` (list) did not exist and `GET /api/defects/:id` was a 501 stub, so F21 Defects Hub rendered blank. New `DefectsService` (`apps/api/src/defects/defects.service.ts`) backs two read endpoints on `DefectsController`: **`GET /api/defects`** — workspace-scoped list with optional `?projectId=&status=&severity=&assigneeId=&component=&q=&page=&pageSize=` filters, ordered `createdAt DESC`, paginated; **`GET /api/defects/:id`** — detail (the F21 row → F22 header data). Both are `@UseGuards(RolesGuard)` + `@Roles(Admin, Lead, QAEngineer, Stakeholder)` — read-only across all 4 roles, parity with the test-cases/requirements list surface (Stakeholder is a read-only role). Tenant isolation: `findMany`/`findFirst` constrained by `project.workspaceId = session.workspaceId` (the proven-live app-level layer; the Day-32 G5 probe showed DB RLS is installed but inert for the `neondb_owner` app role) — cross-workspace or absent → **404** (no existence leak, no 403). **Reads are NOT audited** — ERD §8.7 + Hard Rule 7 scope the audit log to state-changing operations; `DefectsService` has no `AuditService` dependency. `packages/shared/src/schemas/defect.ts` gains the read shapes `DefectListQuery`/`DefectListItem`/`DefectListResponse`/`DefectDetailResponse` (Hard Rule 10). The M4 `component`/`verifiedAt`/`closedAt` columns (on the prisma `Defect` model) are added to **`DefectListItem`** (the read response) rather than the base `DefectSchema` — the base schema is left untouched so the FE `demo-seed.ts` consumer keeps typechecking; surfacing them only on the read item keeps the change in-lane. No DB/migration impact (reads existing seeded rows); requires a Render API redeploy. Tests (TDD, failing-first): 8 service-spec cases (workspace scoping, filter threading, Date→ISO + assignee projection, skip/take pagination, 404 absent + cross-tenant) + 5 controller cases (RBAC metadata on list + detail, query-parse + wiring, 400-on-non-UUID). The `create`/`status`/`jira-push`/`GET :id/rca` endpoints remain 501 stubs (out of scope). DI: `DefectsModule` already imports `AuthModule` (from #262) so the new `DefectsService` provider + the controller's `AuthService`/`RolesGuard` resolve at boot.

### Security — Day 32 (Thu 2026-06-11) — guard /defects/:id/rca + block disabled users [m5-hardening]

Two pre-launch fixes from the Day-32 full BE conformance audit (`docs/audits/2026-06-11-day-32-be-full-conformance-audit.md`). **P1 — RCA kickoff was anonymous:** `POST /api/defects/:id/rca` (`defects.controller.ts`) had no guard, so anyone with a defect UUID could trigger the Sherlock 4-agent LLM fan-out (Groq quota burn) and write `actorId:null` audit rows. Added `@UseGuards(RolesGuard)` + `@Roles(Admin, Lead, QAEngineer)`, wired the real session actor into the audit row + the orchestrator context (replacing the `actorId:null` TODO), and added tenant isolation — a user may only trigger RCA on a defect in their own workspace (cross-tenant → 404, no LLM spend). **P2 — disabled users kept access until session expiry:** `resolveSession` (`auth.service.ts`, the single chokepoint behind `RolesGuard` for all `/api/*` + the `/auth/session` wrapper) now selects `disabledAt` and returns `null` when it is set, so an Admin-disabled user loses API access immediately instead of after their 7-day BetterAuth session expires. Gates **only** on `disabledAt` (NOT `activatedAt` — the seeded roster has `activatedAt=NULL`, so an activatedAt gate would lock out all 8 pilot users). Tests: a failing→passing RBAC-metadata test + a tenant-isolation 404 test on the defects spec; a disabled-blocked + active-allowed pair on the auth.service.day0 spec. Full api suite 60/60 suites · 701/701 tests green. **DI fix-up:** `DefectsModule` now imports `AuthModule` (which exports `AuthService` + `RolesGuard`) — without it Nest cannot resolve the controller's new `AuthService` injection + the `RolesGuard`, so the API fails to boot. Unit tests `useValue`-provide both (so they passed); the gap was caught by the E2E "API failed to boot" check and re-verified locally (Nest mounts `DefectsController {/api/defects}` against a bogus DB, failing only with `P1001` after DI fully resolves). No DB/migration impact; requires a Render API redeploy.

### Fixed — Day 4 (Sun 2026-06-07, pilot-prep) — P0-001 follow-up: session role/displayName fields [pilot-prep]

P0-001 follow-up: after the cross-site cookie fix (#256), cross-site auth worked but `GET /auth/get-session` returned only BetterAuth's `auth_user` core fields (`name`/`email`/`emailVerified`/`image`/`id`) — **no `role`/`displayName`/`organizationalLabel`**, so the FE fell back to a default role (`QAENGINEER`) + email-prefix display name for all 8 pilot users (cosmetic but visible). Root cause: those columns live on the **separate** TB-002 `users` table, not on BetterAuth's `auth_user` table (the `auth_*` tables are intentionally distinct). `additionalFields` can't surface them (they aren't on `auth_user`). Fix: add a BetterAuth `customSession` plugin in `apps/api/src/auth/auth.config.ts` whose `after` hook joins TB-002 by email and merges `role`/`displayName`/`organizationalLabel` (+ `appUserId`/`workspaceId`) into the `/auth/get-session` user object. **No FE change** (the existing `useSessionUser()` hook already maps these when present) and **no schema migration** (reads the canonical `users` source — one indexed `findUnique` per session read). `customSession` runs before `nextCookies()` (which stays last). Requires a Render API redeploy; no DB impact. **Test follow-up (Day-32 Thu):** the two auth specs that fully `jest.mock` `better-auth/plugins` (`t021-auth.config.spec.ts` + `send-magic-link.spec.ts`) had factories exporting only `magicLink`, so the new `customSession` import resolved `undefined` under jest (`customSession is not a function` — a mock factory replaces the real module entirely; NOT an ESM/CJS transform issue, which is why tsc/nest-build/Node-require were all unaffected and production was always safe). Both factories now also export a `customSession` stub; full api suite 60/60 suites · 697/697 tests green.

### Fixed — Day 4 (Sun 2026-06-07, pilot-prep) — P0-001 cross-site auth cookie + /api CORS [pilot-prep]

P0-001 (magic-link sign-in succeeds but the app shows no real identity — it renders the hardcoded "Kishor K." fallback; no cookie stored on the FE) root-caused to the cross-site pilot topology: FE `qa-nexus-web.pages.dev` + API `qa-nexus-api.onrender.com` are **different registrable domains**, but the auth config was written for a future shared-parent zone (`*.qanexus.iksula.com`). Two BE fixes: (1) `apps/api/src/auth/auth.config.ts` — `resolveCookieDomain` → `resolveCookieConfig`, now per-topology: for a host NOT under `.qanexus.iksula.com` it emits a **host-only** cookie (no `Domain`) with `SameSite=None; Secure; Partitioned` (CHIPS) instead of the previous `Domain=.qanexus.iksula.com` + `SameSite=Lax` (the browser rejected that Domain since it ≠ the origin host, and `Lax` is never sent on cross-site fetch). (2) `apps/api/src/main.ts` — added credentialed CORS for `/api/*` (previously only `/auth/*` had it, so every cross-origin data fetch was blocked with no `Access-Control-Allow-Origin`), reusing `isAuthCorsOriginAllowed` so the `/api` + `/auth` allowlists stay in sync. Shared-parent + localhost topologies are unchanged (still `SameSite=Lax`, wildcard/host-only respectively). Security: `SameSite=None` is mitigated by BetterAuth `trustedOrigins` CSRF (pages.dev is listed) + CHIPS partitioning — acceptable for the pilot; the long-term fix is `*.qanexus.iksula.com` custom domains. Requires a Render API redeploy; no DB/migration impact. Pairs with the FE-side `credentials:'include'` + `GET /auth/session` identity wiring. RCA: `docs/pilot-prep/2026-06-07-p0-001-cookie-cors-root-cause-be.md`.

### Changed — Day 3+4 (Sat 2026-06-06, pilot-prep) — NFR-003 production verification deferred to Day-29 [pilot-prep]

The `/admin/nfr/{a1,a2}` probes are deployed + live on Render (401 unauthenticated → route + RolesGuard active), but production A1/A2 measurement is **deferred to Day-29**: capturing a cross-origin BetterAuth session cookie was blocked (the FE home renders 100% stub data → no API call to lift a session from; CORS blocks `/admin/nfr/*` from the browser console). Resolution filed in `feedback_nfr_probe_token_auth.md` — add an optional `NFR_PROBE_TOKEN` + `X-NFR-Probe-Token` header bypass (no cookie, no CORS) for a ~1-line Day-29 cleanup PR. `docs/pilot-prep/m5-nfr-baseline.md` §Production Measurements marked DEFERRED (A1 < 10 s engineered estimate · A2 ~150 ms 14th-finding projection · A4 RESOLVED via ADR-024 pilot p95 < 20 s). Task 6 (AC011/AC021) eval approach + quota/timing drafted in `docs/pilot-prep/2026-06-06-ac011-ac021-eval-prep.md`.

### Added — Day 3+4 (Sat 2026-06-06, pilot-prep) — Apps Script email provider strategy (ADR-025) [pilot-prep]

`EmailService` refactored to a provider strategy (ADR-025): an `EmailProvider` interface + `ResendEmailProvider` (the ADR-018 logic extracted verbatim — BCC, reply-to, `RESEND_API_KEY` redaction) + a new `AppsScriptEmailProvider` (POSTs `APPS_SCRIPT_EMAIL_URL` with a shared secret; `AbortController` 10s timeout; secret-sanitized errors; plain-text fallback for deliverability). `EMAIL_PROVIDER` selects the active provider (default `apps-script`) with the other as fallback; the public API (`sendInvitation`/`sendMagicLink`/`sendPasswordReset`/`send`/`getHealth`) **and the BetterAuth magic-link wiring are unchanged**. Pilot magic-links now route through a Google Apps Script Web App (Workspace sender `yogesh.mohite@iksula.com`, 1500/day, $0, no DNS) — every transactional-email service needs DKIM/SPF on the IT-controlled `iksula.com` domain, and Resend Pro ($20/mo) violates the $0 gate. Migrating back to Resend is a pure env swap (`EMAIL_PROVIDER=resend`) once `mail.qanexus.iksula.com` verifies. Zod `AppsScriptEmailRequest`/`AppsScriptEmailResponse` in `packages/shared`. 31 email tests green (provider-selection + isolated Apps Script provider with mocked fetch).
