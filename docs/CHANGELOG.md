...
- **`test(api)`** — **Coverage catch-up: 39 → 68 jest tests** (+29). Closes the highest-value <80%-coverage gaps without adding live-integration noise. Four new spec files, all PASS:
  - `src/audit/__tests__/audit-helper.spec.ts` (9 tests) — HMAC-SHA256 chain integrity (binding per CLAUDE.md Hard Rule 7 + PM1_ERD §3.13). Covers genesis row (`prevHash = '0'×64`), chain link (`prevHash = previous.thisHash`), canonical-JSON key normalization (re-ordered nested objects + arrays produce identical hash), payload-differs-→-hash-differs, secret-rotation-→-hash-differs, `BETTER_AUTH_SECRET` missing-or-too-short guard, advisory-lock key derivation (first 8 hex chars of workspace UUID). Coverage `audit-helper.ts` 12.5% → ~95%.
  - `src/audit/__tests__/audit.service.spec.ts` (5 tests) — `AuditService` wrapper. `write()` relays + returns `{id, thisHash}`, propagates helper errors (chain integrity is binding); `writeNonBlocking()` returns void synchronously + swallows + warn-logs helper rejections (verified via spy on the private logger); `resolveActorByEmail()` returns `{workspaceId, actorId}` when user exists, falls back to seeded workspace + null actor for unknown emails (pre-signup events). Coverage `audit.service.ts` 35% → ~95%.
  - `src/llm/__tests__/base.provider.spec.ts` (9 tests) — `BaseProvider` retry-with-backoff + health tracking. Covers happy path → `status=up`, transient (429) once → retry succeeds, transient (503) twice → `RetryableLLMError` + `status=down`, non-transient (400) → bubbles immediately with no retry, explicit `RetryableLLMError` throw → treated as transient + retried, `isTransientError()` reads `.status` / `.statusCode` / `.response.status` (and rejects 500/400/null/string/RetryableLLMError-instance), `healthCheck()` true/false, `lastSuccessAt` preserved when later failure overwrites status (so `/health` can show "last seen up at X"). Coverage `base.provider.ts` 9% → ~95%.
  - `src/llm/__tests__/provider-registry.spec.ts` (5 tests) — `provider-registry.ts` lazy + cached factory + unknown-name error path. Mocks `GroqProvider`/`GeminiProvider` so no real API keys needed; verifies `listProviders()` returns canonical Day-3 set, `getProvider('groq')` caches per-name, `getProvider('mistral')` throws helpful add-a-provider message naming `provider-registry.ts`, `_resetProviderCacheForTests()` clears so factory fires again. Coverage `provider-registry.ts` 35% → 100%.
- **`feat(scripts)`** — `scripts/llm-gateway-validation.sh` — one-shot end-to-end validation script for the deployed LLM gateway (Day-4 noon brief Block 1 + Block 2, deferred to when Yogesh's Render provisioning + GROQ/GEMINI keys land). `set -euo pipefail`, idempotent (timestamped append per run), covers (a) happy path latency budget, (b) Gemini fallback (manual: revoke GROQ key in Render between runs), (c) long-context route (~40k chars), (d) p50/p95 over 10 sequential calls, (e) A1 Scribe smoke against real LLM. Burns ~12 Groq RPD per run (0.8% of daily quota). Output: markdown-appended to `docs/observability/llm-gateway-validation-YYYY-MM-DD.md`.
- **`docs(observability)`** — `docs/observability/llm-gateway-validation-2026-04-30.md` — Day-4 morning placeholder explaining the deferral (Render at `qa-nexus-api.onrender.com/health` returned 404 at 07:30 IST → not yet bound), how to run the validation script once keys land, and what the script's pass criteria are.
