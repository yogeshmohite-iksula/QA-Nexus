...
| 5   | **#168**                 | feat(api): Sherlock RCA orchestrator + 3 sibling agents + DefectsController.rca [M4 TASK A4-A8] | **MERGEABLE/CLEAN** ✅ (chained-base) | auto-retargets to main when #161 lands               | BE+1 promoted 16:25 IST. 1281+/60- across 15 files. Chained-base `feat/be-sherlock-code-agent` (PR #161). 543/543 jest pass. |
| 6   | **(new)** F20 React port | FE+1 today; first production use of skill v2.1.2 + AdminShell `data-canonical-section` attrs    | _not yet opened_                      | TBD (likely CHANGELOG.md after #161/#162/#168 merge) | FE+1 ETA ~17:30 IST after visual gate                                                                                        |
...
- **#168 fifth** — Sherlock orchestrator depends on #148 (event emission) + #161 (one agent pattern). Auto-retargets to main after #161 lands via GitHub's chained-base mechanic.
- **#6 last** — F20 React port consumes BE endpoints from #149/#148/#161/#162/#168. Merging last keeps FE re-rebase scope minimal if a BE PR slips.

**Chained-base mechanics (PR #168):** PR #168's base is `feat/be-sherlock-code-agent` (the head of PR #161), NOT `main`. When #161 squash-merges into main, GitHub auto-retargets #168's base to `main` and recomputes mergeable. **DO NOT manually rebase #168 onto main pre-#161-merge** — that would lose the chained-PR review benefit. Just merge #161 first; #168's MERGEABLE/CLEAN state will carry through the auto-retarget. Allow ~30s for GitHub to recompute before merging #168.

**BE+1 scope cuts in #168 (documented followup `(da)`):** Synchronous orchestrator contract for M4 close; async DB persistence + queue-backed retries + per-defect audit-row writes deferred to Day-21 hardening. AC042 corpus eval still runs Day-21 against the synchronous-only orchestrator (200 LLM calls, no DB cost). ADR-019's 4-agent parallel fan-out pattern is fully live; only the persistence/audit/queue layers are deferred — the RCA quality bar (AC042 ≥40% top-2) is gated on the orchestrator behavior itself, not the persistence path.
