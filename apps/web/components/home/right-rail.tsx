...
function PinnedCard({ pin, onRoute }: { pin: PinnedRef; onRoute: (t: string) => void }) {
  const isApproved = pin.status === 'approved';
  return (
    <li>
      <button
        type="button"
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2 text-[11px]">
          {isApproved ? (
            ...
            <span className="inline-flex items-center rounded border border-[var(--border-subtle)] bg-[var(--overlay)] px-1.5 py-0.5 font-mono text-[var(--text-tertiary)]">
              Reference
            </span>
          )}
