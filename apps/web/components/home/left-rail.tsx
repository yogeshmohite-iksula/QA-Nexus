// Left navigation rail for F08a Home (QA Engineer).
//
// Hidden below lg: (1024 px) — folds away into a future drawer/hamburger
// (TODO when F08c stakeholder/lead variants land). At lg+, sits at 240 px
// wide with active-item violet 3px lane on the left edge.
//
// Role-gated: QA Engineer does NOT see the QA Value or Govern sections
// (per the locked frame's HTML comment "QA Value HIDDEN" / "ENTIRE GOVERN
// SECTION HIDDEN for QA Engineer"). Those sections are simply omitted from
// the data array below; F08c/F08d ports will use a different array.
...
const SECTIONS: NavSection[] = [
  ...
  },
  {
    ...
    title: 'Plan',
    items: [
      { id: 'requirements', label: 'Requirements', count: { value: '142', tone: 'neutral' } },
      { id: 'test-plans', label: 'Test Plans & Cycles' },
      { id: 'test-cases', label: 'Test Cases', count: { value: '1,284', tone: 'neutral' } },
    ],
  },
  {
    id: 'author',
  ...
  },
  {
    ...
    title: 'Run',
    items: [
      { id: 'runs-sessions', label: 'Runs & Sessions', count: { value: '1 live', tone: 'pass' } },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'defects', label: 'Defects / Failures', count: { value: '2', tone: 'fail' } },
...
function NavLink({ item }: { item: NavItem }) {
  return (
    ...
    >
      {/* Active lane indicator (violet 3px on left edge) */}
      {item.active && (
        <span
          aria-hidden="true"
      ...
      {item.badge && (
        <span className="rounded border border-[var(--border-subtle)] bg-[var(--overlay)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--text-tertiary)]">
          {item.badge}
        </span>
      )}
    </button>
...
function CountChip({
  ...
  const toneClass: Record<typeof tone, string> = {
    ...
    neutral: 'text-[var(--text-tertiary)] bg-[var(--overlay)]',
  };
  return (
    <span
      className={[
