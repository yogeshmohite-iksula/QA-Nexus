// Implements F06b Set Password · see PM1_UI_v2/frames - claude code build (PM1 v2.6-v2.8)/F06b Set Reset Password.html
// Backend wiring deferred to MS0-T021 (BetterAuth integration).
//
// F06b is the INVITE-LINK LANDING (Mode A): user clicks magic link from
// invite email, lands here, sets a new password to activate account.
// F06c (Reset Password) is the FORGOT-FLOW landing — separate route at
// /sign-in/forgot (currently a stub, full UI in MS0-T029 successor).
//
...
//     right, each flex-1.
// The locked HTML F06b frame is a 1600x1024 design REFERENCE -- React
// port is fully responsive per PM1_PRD §10 NFR-001.
//
// Anti-drift per validation checklist line 504: NO violet UI here (Auth surface
// is AI-free). Violet reserved for focus-rings only. TEAL for system actions.
...
export default function SetPasswordPage() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    // MS0-T021 (M1): wire BetterAuth POST /auth/set-password here.
    // For now: no-op stub so the form renders and submits cleanly.
    console.warn('F06b Set Password submit -- BetterAuth integration pending (MS0-T021)', {
  ...
  return (
    <div className="flex min-h-screen flex-col lg:flex-row" id="stage">
      ...
      {/* RIGHT AUTH PANEL — F06b Mode A Invite Setup
          Notes:
          - F06b form is taller than F06 (Welcome + 2 pw fields + strength card + footer).
            Use overflow-y-auto on the panel so short viewports scroll the form instead
            of cramping content edge-to-edge.
          - DEVIATION from locked HTML (lines 338-346): the locked F06b ships a small
            QA Nexus wordmark in the top-left of this panel as well. Per Yogesh override
            on 2026-04-26, that wordmark is OMITTED on desktop — the left brand panel
            already shows QA Nexus prominently; a second instance is redundant. On
            mobile (< lg) the BrandMark renders ABOVE the form to preserve identity
            when the brand panel is hidden by the responsive layout. Same deviation
            applies to all future auth-surface frame ports (F06c, F07, F07b/c/d). */}
      <section
        ...
        <div className="w-full max-w-[440px]">
          ...
          <div>
            <h2
              ...
              style={{ fontFamily: 'var(--font-dm-sans), system-ui, sans-serif' }}
            >
              Welcome, <span className="text-primary font-bold">{DEMO_INVITEE_NAME}</span>!
            </h2>
            <p className="text-text-secondary m-0 text-[15px] font-normal leading-6 sm:text-[16px]">
              Set a password to activate your QA Nexus account.
            </p>
          </div>
          ...
          <form className="mt-8 flex flex-col gap-3" onSubmit={handleSubmit} noValidate>
            ...
            <PasswordStrengthCard className="mt-2" />
          ...
          <div className="mt-6 flex items-center justify-center gap-2">
            ...
            >
              Link expires in{' '}
              <span className="text-text-secondary font-semibold">23 hours 14 minutes</span>
            </p>
          </div>
        </div>
      </section>
