...
//
// Schedule per ADR-021 Amendment A:
//   - 02:30 IST daily — pre-compute all (kind × project × window) into
//     report_aggregate. Tier-1 of the hybrid 3-tier strategy.
//   - Every 15 min — stale sweep. Defensive backup for event-triggered
//     invalidation; flips is_stale=true on rows >24h old.
//
// 02:30 IST (not 02:00) avoids overlap with the weekly pg_dump cron
// (MS0-T018 — Sunday 03:00 IST).
//
...
export class ReportsRefreshCron implements OnModuleInit {
  ...
  onModuleInit(): void {
    this.logger.log(
      'ReportsRefreshCron: 02:30 IST daily + 15-min stale sweep registered',
    );
  }

  /// 02:30 IST daily — pre-compute all tier-1 aggregates.
  ///
  ...
  /// Every 15 min — mark rows older than 24h as stale.
  ...
  // Every 15 min (cron expression — CronExpression.EVERY_15_MINUTES enum
  // is not present in current @nestjs/schedule; use literal cron string).
  @Cron('*/15 * * * *')
